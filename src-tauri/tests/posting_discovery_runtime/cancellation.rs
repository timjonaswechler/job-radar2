...
#[test]
fn posting_discovery_cancellation_interrupts_an_active_http_fetch_without_fallback_failure() {
    block_on(async {
...
#[test]
fn posting_discovery_browser_cancellation_is_distinct_from_runtime_failure() {
    block_on(async {
...
#[test]
fn posting_discovery_cancellation_stops_page_pagination_before_the_next_request() {
    block_on(async {
...
impl PostingDiscoveryFetcher for CancelAfterFirstPageFetcher<'_> {
    fn fetch<'a>(
        &'a self,
        _request: PostingDiscoveryFetchRequest,
    ) -> Pin<
        Box<
...
impl PostingDiscoveryFetcher for HangingFetcher {
    fn fetch<'a>(
        &'a self,
        _request: PostingDiscoveryFetchRequest,
    ) -> Pin<
