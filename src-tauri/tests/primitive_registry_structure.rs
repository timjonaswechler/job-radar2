...
fn top_level_registry_only_declares_family_modules() {
    let source = include_str!("../crates/source-profile-dsl/src/profile_dsl/primitives/mod.rs");
...
fn completeness_assembly_has_no_productive_dependency_or_dispatch() {
    let source =
        include_str!("../crates/source-profile-dsl/src/profile_dsl/primitives/completeness.rs");
    for forbidden in [
    ...
    let model = include_str!(
        "../crates/source-profile-dsl/src/profile_dsl/primitives/completeness/model.rs"
    );
