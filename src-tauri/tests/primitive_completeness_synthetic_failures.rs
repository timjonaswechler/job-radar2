...
fn schema(key: &str) -> SchemaShape {
    SchemaShape {
        ...
        owner: Owner::P06bc,
        canonical_file: "src-tauri/src/profile_dsl/primitives/value/mod.rs".into(),
...
fn serde(key: &str) -> SerdeShape {
    SerdeShape {
        ...
        compiled_identity: identity(key),
        shape: Tagged,
        authored_file: "src-tauri/src/profile_dsl/documents/extract.rs",
    }
}
...
fn policy(forbidden: &[(Family, &str)]) -> CompletenessPolicy {
    CompletenessPolicy {
        ...
        canonical_files: BTreeMap::from([
            ...
            (
                Owner::P02,
                BTreeSet::from(["src-tauri/src/profile_dsl/primitives/parse/mod.rs".to_owned()]),
            ),
        ]),
    }
}
...
fn duplicate_conflicting_owner_context_shape_file_and_behavior_fail() {
    ...
    let mut v = complete("template");
    v.compiled[0].canonical_file = "src-tauri/src/profile_dsl/compiler/mod.rs";
    assert!(errors(&v)
        .iter()
...
fn compiled_identities_are_non_empty_and_globally_unique() {
    ...
    missing_witness.compiled[0].witness = job_radar_lib::missing_witness;
...
fn every_nested_shape_class_and_removed_key_is_visible() {
    ...
    let removed = PrimitiveCompletenessInventories {
        schema: vec![SchemaShape {
            ...
            contexts: ctx.clone(),
            owner: Owner::P02,
            canonical_file: "src-tauri/src/profile_dsl/primitives/parse/mod.rs".into(),
