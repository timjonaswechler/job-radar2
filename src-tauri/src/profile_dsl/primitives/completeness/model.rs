...
#[derive(Clone)]
pub(crate) struct OwnerClassification {
    pub owner: Owner,
    pub canonical_file: String,
    pub compiled_identity: String,
}
...
pub(crate) fn frozen_owner_classification(
    layer: InventoryLayer,
    family: Family,
    key: &str,
    ...
    let catalogue = match layer {
        InventoryLayer::Schema => {
            include_str!("../../../../../docs/agents/primitive-schema-owner-catalogue.txt")
        }
        InventoryLayer::Serde => {
            include_str!("../../../../../docs/agents/primitive-serde-owner-catalogue.txt")
        }
...
fn canonical_file(policy: &CompletenessPolicy, owner: Owner, file: &str) -> bool {
    file.starts_with("src-tauri/src/")
        && !file.contains("..")
        && policy
            .canonical_files
            .get(&owner)
