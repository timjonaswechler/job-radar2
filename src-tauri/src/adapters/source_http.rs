    ...
    ) -> Pin<Box<dyn Future<Output = Result<ProfileHttpResponse, ProfileHttpError>> + Send + 'a>>
    {
        Box::pin(async move {
            let method = match request.method {
                source_engine::definition::HttpMethod::Get => reqwest::Method::GET,
                source_engine::definition::HttpMethod::Post => reqwest::Method::POST,
            };
...
mod tests {
    use super::*;
    use source_engine::definition::HttpMethod;
    use std::{
        io::{Read, Write},
        net::TcpListener,
        thread,
