...
use crate::definition::documents::discovery::{
...
use crate::definition::primitives::acceptance::{
    compile_acceptance, AcceptanceCompileContext, CompiledAcceptance,
};
use crate::definition::primitives::capture::{
    compile_captures, CaptureCompileError, CompiledCapturePlan,
};
use crate::definition::primitives::pagination::{compile_pagination_plan, CompiledPagination};
use crate::definition::primitives::parse::{compile_parse, CompiledParse, ParseInputKind};
...
use crate::definition::primitives::select::{
    compile_select, CompiledSelect, SelectCompileContext, SelectPhase, SelectPlacement,
};
use crate::definition::primitives::value::{
    CompiledListValue, CompiledValue, ValueCompileContext, ValueCompileError, ValuePlacement,
};
use crate::definition::template::{json_pointer_segment, TemplateAdmissionKeys, TemplatePlacement};
use crate::execution::occurrence::HintUse;
...
pub fn compile_discovery_step(
    ...
    let acceptance_context = AcceptanceCompileContext::discovery(posting_meta_keys);
    Ok(ExecutionPlanDiscoveryStep {
        ...
        accept_when: step
            ...
            .as_ref()
            .map(|acceptance| {
                compile_acceptance(acceptance, &acceptance_context)
...
fn compile_discovery_strategy(
    ...
    let fetch = compile_fetch(
        ...
        match strategy.fetch {
            crate::definition::documents::Fetch::Http { .. } => TemplatePlacement::DiscoveryHttpUrl,
            crate::definition::documents::Fetch::Browser { .. } => {
    ...
    Ok(ExecutionPlanDiscoveryStrategy {
        ...
        parse: compile_parse(
            &strategy.parse,
            match strategy.fetch {
                crate::definition::documents::Fetch::Http { .. } => ParseInputKind::DecodedHttp,
                crate::definition::documents::Fetch::Browser { .. } => {
        ...
        accept_when: strategy
            ...
            .map(|acceptance| {
                compile_acceptance(
                    acceptance,
                    &AcceptanceCompileContext::discovery(
                        strategy
                            .extract
                            .posting_meta
                            .as_ref()
...
fn compile_discovery_extraction(
    ...
    let compile_optional = |value: Option<&crate::definition::documents::FieldExpression>,
                            field_path: String| {
        value
            .map(|value| {
    ...
    let compile_map = |values: Option<
        &std::collections::BTreeMap<String, crate::definition::documents::FieldExpression>,
    >,
                       section: &str| {
        values
            .map(|values| {
