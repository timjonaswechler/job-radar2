...
pub fn value_expression_node_count(expression: &FieldExpression) -> usize {
    match expression {
...
fn compile_pipeline(
    transforms: &[crate::definition::primitives::transform::Transform],
    path: &str,
...
fn transform_output_shape(transforms: &CompiledTransformPipeline, input: ValueShape) -> ValueShape {
    use crate::definition::primitives::transform::TransformShapeKind;
...
pub fn compile_list_value(
    expression: &crate::definition::documents::ListFieldExpression,
    context: &ValueCompileContext,
) -> Result<CompiledListValue, ValueCompileError> {
    match expression {
        crate::definition::documents::ListFieldExpression::Single(value) => {
            Ok(CompiledListValue::Single(compile_value(value, context)?))
        }
        crate::definition::documents::ListFieldExpression::Multiple(values) => {
...
pub fn completeness_compiled_registrations(
    ...
    const ALL: &[crate::definition::primitives::completeness::PrimitiveContext] = &[
        DiscoveryCaptureSource,
        DiscoveryFilterOutput,
        DetailCaptureSource,
        DetailMatchFilterOutput,
        ...
        ($key:literal,$variant:literal,$file:literal,$shape:expr,$witness:expr) => {
            CompiledRegistration {
                ...
                canonical_file: concat!(
                    "src-tauri/crates/source-engine/src/definition/primitives/value/",
                    $file,
                    ".rs"
                ),
                shape: $shape,
    ...
    macro_rules! local {
        ($key:literal,$contexts:expr,$shape:expr,$identity:literal,$witness:expr) => {
            CompiledRegistration {
                ...
                owner: P06bc,
                canonical_file:
                    "src-tauri/crates/source-engine/src/definition/primitives/value/mod.rs",
                shape: $shape,
    ...
    out.extend([
        CompiledRegistration {
            ...
            owner: P06a,
            canonical_file: "src-tauri/crates/source-engine/src/definition/primitives/value/mod.rs",
            shape: ParentOption,
