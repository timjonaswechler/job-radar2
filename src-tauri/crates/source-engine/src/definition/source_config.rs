...
#[derive(Clone, Debug)]
pub struct EffectiveSourceConfigContract {
...
impl EffectiveSourceConfigContract {
    ...
    pub fn requires_property(&self, name: &str) -> bool {
        self.required.contains(name)
    }

    pub fn validate_incremental(&self, values: &JsonObject) -> Vec<ContractViolation> {
        self.validate(values, false)
    }

    pub fn validate_complete(&self, values: &JsonObject) -> Vec<ContractViolation> {
...
pub fn is_search_request_criteria_key(key: &str) -> bool {
    let normalized = key
        .chars()
