...
use super::{
    browser_acquisition::{
        ...
        BrowserAcquisitionTerminal, BrowserInteractionInstruction, BrowserWaitInstruction,
    },
    cancellation::{
...
pub(crate) async fn execute_canonical_browser_fetch(
    ...
    let request = match BrowserAcquisitionRequest::new(
        ...
        input
            .waits
            .iter()
            .map(BrowserWaitInstruction::from_plan)
            .collect(),
        input
            .interactions
            .iter()
            .map(BrowserInteractionInstruction::from_plan)
            .collect(),
        input.control,
    ) {
        Ok(request) => request,
