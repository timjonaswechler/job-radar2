...
use crate::{
    definition::{compiler::CompiledSource, diagnostics::Diagnostics},
...
pub struct SourceBehaviorDetailExecution<'a, F: ?Sized, A: ?Sized> {
    fetcher: &'a F,
    acquisition: &'a A,
}

impl<'a, F: ?Sized, A: ?Sized> SourceBehaviorDetailExecution<'a, F, A> {
...
impl<F, A> SourceDetailExecution for SourceBehaviorDetailExecution<'_, F, A>
where
...
impl<F, A> SourceBehaviorDetailExecution<'_, F, A>
where
    F: ProfileHttpClient + Sync + ?Sized,
    A: BrowserAcquisition + Sync,
{
