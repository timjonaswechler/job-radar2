...
pub(super) async fn execute_single_strategy_fetch<F>(
    ...
    total_path: Option<&crate::definition::primitives::select::JsonPathSelectPlan>,
    next_cursor_path: Option<&crate::definition::primitives::select::JsonPathSelectPlan>,
    base_path: &str,
...
fn extract_total_count(
    ...
    total_path: &crate::definition::primitives::select::JsonPathSelectPlan,
) -> Option<u64> {
    let document::ParsedDocument::Json(value) = document else {
        return None;
    };
    match crate::definition::primitives::select::resolve_compiled_json_path(total_path, value)? {
...
fn extract_next_cursor(
    document: &document::ParsedDocument<'_>,
    next_cursor_path: &crate::definition::primitives::select::JsonPathSelectPlan,
) -> Option<String> {
    let document::ParsedDocument::Json(value) = document else {
        return None;
    };
    let value = match crate::definition::primitives::select::resolve_compiled_json_path(
        next_cursor_path,
        value,
    )? {
