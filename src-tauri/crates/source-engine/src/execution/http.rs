...
use crate::definition::documents::HttpMethod;
...
fn normalized_transport_headers(
    headers: impl IntoIterator<Item = (String, Vec<u8>)>,
) -> Vec<ProfileHttpHeader> {
...
/// Completes the adapter-neutral HTTP contract from transport response parts.
///
/// Concrete transport adapters use this function so streaming byte allowances,
/// cancellation, deadlines, charset handling, and response normalization remain
/// owned by the Source Behavior Language runtime.
#[doc(hidden)]
pub async fn collect_profile_http_response<S>(
    stream: S,
    status: u16,
    final_url: String,
    headers: Vec<(String, Vec<u8>)>,
...
{
    let headers = normalized_transport_headers(headers);
...
mod tests {
    use super::*;
    use crate::{
        definition::documents::PhaseLimits,
        execution::{
            allowance::{
