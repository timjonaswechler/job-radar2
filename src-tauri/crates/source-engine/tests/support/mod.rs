...
impl source_engine::test_support::SourceProfileLookup for SourceProfileRegistrySnapshot {
...
pub struct AcceptedPhase<P> {
    pub payload: P,
    pub diagnostics: source_engine::test_support::Diagnostics,
...
pub async fn execute_discovery_test<F>(
    ...
    fetcher: &F,
) -> AcceptedPhase<source_engine::test_support::DiscoveryPhasePayload>
where
...
pub async fn execute_detail_test<F>(
    ...
    fetcher: &F,
) -> AcceptedPhase<source_engine::test_support::DetailPhasePayload>
where
    F: ProfileHttpClient + Sync + ?Sized,
{
