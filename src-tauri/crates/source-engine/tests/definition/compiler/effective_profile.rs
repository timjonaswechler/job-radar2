use crate::compiler_support::{
    RegistrySource, RegistrySourceProfile, SourceProfileRegistrySnapshot, SourceValidationState,
    ValidationStateKind,
};
...
fn profile_source_compiles_to_a_complete_effective_profile_and_plan() {
    ...
    let CompiledSourceAccess::Profile { effective_profile } =
        source_engine::test_support::test_access(&compiled)
    else {
...
fn directly_supplied_source_is_authoritative_over_same_key_registry_source() {
    ...
    let serialized_plan =
        serde_json::to_string(&source_engine::test_support::test_execution_plan(&compiled))
            .unwrap();
...
fn source_owned_access_is_a_distinct_complete_branch() {
    ...
    let CompiledSourceAccess::SourceOwned { access_path } =
        source_engine::test_support::test_access(&compiled)
    else {
