...
fn profile_value() -> serde_json::Value {
    fixture("../../tests/fixtures/source-behavior/valid/simple-source-profile.json")
}
...
fn compile_source_compiles_value_http_browser_and_detection_templates_into_typed_plans() {
    ...
    let serialized =
        serde_json::to_string(&source_engine::test_support::test_execution_plan(&source)).unwrap();
    assert!(serialized.contains("reference"));
    assert!(serialized.contains("sourceConfig"));
    assert!(!serialized.contains("{{sourceConfig"));
    let plan = source_engine::test_support::test_execution_plan(&source);
    let CompiledValue::Template { template, .. } = plan.discovery.strategies[0]
        .extract
        .output
        .provider_values
        .as_ref()
...
fn compile_source_rejects_detail_fetch_capture_before_io_and_detection_malformed_template() {
    ...
    let detection_diagnostics =
        source_engine::test_support::compile_detection_plan(&profile).unwrap_err();
