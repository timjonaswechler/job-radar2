...
fn serde(key: &str) -> SerdeShape {
    SerdeShape {
        ...
        compiled_identity: identity(key),
        shape: Tagged,
        authored_file: "src-tauri/crates/source-engine/src/definition/documents/extract.rs",
    }
}
    ...
    CompletenessPolicy {
        ...
        canonical_files: BTreeMap::from([
            ...
            (
                Owner::P02,
                BTreeSet::from([
                    "src-tauri/crates/source-engine/src/definition/primitives/parse/mod.rs"
                        .to_owned(),
                ]),
            ),
        ]),
...
fn duplicate_conflicting_owner_context_shape_file_and_behavior_fail() {
    ...
    let mut v = complete("template");
    v.compiled[0].canonical_file = "src-tauri/crates/source-engine/src/definition/compiler/mod.rs";
    assert!(errors(&v)
        .iter()
...
fn compiled_identities_are_non_empty_and_globally_unique() {
    ...
    missing_witness.compiled[0].witness = source_engine::test_support::missing_witness;
