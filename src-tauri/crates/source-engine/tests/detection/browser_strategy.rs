...
fn d03_descriptor_ties_authored_and_compiled_browser_shape() {
    ...
    assert_eq!(
        ...
        vec![
            source_engine::test_support::DETECTION_URL_DESCRIPTOR,
            DETECTION_BROWSER_DESCRIPTOR
        ]
    );
}
...
async fn http_and_browser_use_independent_shared_report_dimensions() {
    ...
    let http = ScriptedProfileHttpClient::new([
        source_engine::test_support::ScriptedHttpEvent::Response {
            status: 200,
            final_url: "https://example.test/probe".into(),
            headers: vec![],
            body: vec![source_engine::test_support::ScriptedHttpBodyEvent::Chunk(
                b"http".to_vec(),
            )],
...
async fn http_time_does_not_consume_browser_duration() {
    ...
    let http = Arc::new(ScriptedProfileHttpClient::new([
        source_engine::test_support::ScriptedHttpEvent::Response {
            ...
            headers: vec![],
            body: vec![
                source_engine::test_support::ScriptedHttpBodyEvent::Gate("http".into()),
                source_engine::test_support::ScriptedHttpBodyEvent::Chunk(b"http".to_vec()),
            ],
            content_length: None,
        },
    ]));
...
async fn logical_wait_scopes_accept_32_and_deny_a_strategy_fifth_wait() {
    ...
    let compiled_waits = (0..4)
        .map(|_| source_engine::test_support::BrowserWaitInstruction::NetworkIdle { timeout_ms: 1 })
        .collect::<Vec<_>>();
    let mut plans = Vec::new();
