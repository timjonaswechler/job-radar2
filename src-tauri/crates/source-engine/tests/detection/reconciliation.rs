...
fn evidence_identity_uses_kind_and_descriptor_path_and_keeps_origin_order() {
    ...
    let first = DetectionEvidenceContribution::new(
        source_engine::test_support::DetectionEvidenceKind::Http,
...
fn independent_profile_conflict_does_not_hide_matches_but_control_terminals_do() {
    ...
    unsupported_profile.support.level = source_engine::test_support::SupportLevel::Unsupported;
    ...
    let unsupported = match unsupported_context
        ...
        .unwrap()
    {
        source_engine::test_support::PreparedDetectionOutput::Unsupported(unsupported) => {
            unsupported
        }
...
fn profile_evidence_is_seeded_once_and_strategy_recommendations_select_a_path() {
    ...
    profile.detection.as_mut().unwrap().evidence =
        Some(vec![source_engine::test_support::DetectionEvidence {
            kind: source_engine::test_support::DetectionEvidenceKind::Url,
