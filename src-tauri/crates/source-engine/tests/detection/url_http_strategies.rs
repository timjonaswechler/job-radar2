...
fn d02_descriptor_catalogue_ties_authored_and_compiled_url_http_shapes() {
    ...
    let alternatives = DetectionUrlInput::PatternAlternatives {
        alternatives: vec![source_engine::test_support::InputUrlPattern {
            pattern: "(?<tenant>.+)".into(),
    ...
    let option_inventory = |descriptor: &source_engine::test_support::DetectionShapeDescriptor| {
        descriptor
            .options
            .iter()
            .map(|option| {
    ...
    let descriptor_keys = |descriptor: &source_engine::test_support::DetectionShapeDescriptor| {
        descriptor
            .options
            .iter()
...
fn compiler_requires_exact_all_required_url_first_and_compiles_patterns_before_io() {
    ...
    wrong_policy.detection.as_mut().unwrap().policy =
        Some(source_engine::test_support::StrategyPolicy::FirstAccepted);
    assert_eq!(
...
fn direct_serde_rejects_partial_or_mixed_final_detection_shapes() {
    for detection in [
        ...
        assert!(
            serde_json::from_value::<source_engine::test_support::DetectionDocument>(detection)
                .is_err()
        );
    }
...
fn direct_serde_enforces_d02_schema_bounds_and_string_shapes() {
    for key in ["", "Bad", "bad-key"] {
        ...
        .is_err());
        assert!(
            serde_json::from_value::<source_engine::test_support::DetectionDocument>(json!({
    ...
    for evidence in [
        ...
        assert!(
            serde_json::from_value::<source_engine::test_support::DetectionEvidence>(evidence)
                .is_err()
        );
    }
    ...
    assert!(
        serde_json::from_value::<source_engine::test_support::InputUrlPattern>(json!({
            "pattern": ""
        }))
        .is_err()
    );
...
async fn alternatives_feed_latest_reconciled_capture_to_http_template_and_preserve_non_2xx() {
    ...
    assert!(proposal
        .evidence
        .iter()
        .any(|item| item.kind == source_engine::test_support::DetectionEvidenceKind::Http));
    ...
    assert_eq!(
        serde_json::from_value::<source_engine::test_support::DetectionOperationResult>(serialized)
            .unwrap(),
        result
    );
}
