use crate::support::{
    accepted_phase, budget_exhausted, cancelled, execution_failed, not_started,
...
fn caller_raise_is_execution_failed_before_work_with_zero_usage_report() {
    ...
    let result = execution_failed(
        crate::support::block_on(execute_discovery(
            &plan,
            empty_source_config(),
            fetcher.client(),
            PhaseBrowser::BrowserFree,
...
fn sitemap_fan_out_plan() -> source_engine::test_support::SourceExecutionPlan {
...
fn plan() -> source_engine::test_support::SourceExecutionPlan {
...
fn compile_plan(
...
) -> source_engine::test_support::SourceExecutionPlan {
    ...
    match source_engine::test_support::compile_source(&source, &registry) {
        CompileSourceOutcome::Compiled {
            source,
            diagnostics,
        } if diagnostics.is_empty() => source_engine::test_support::test_execution_plan(&source),
