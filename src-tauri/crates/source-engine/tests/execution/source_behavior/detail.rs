...
fn detail_acceptance_rejects_unrequested_fields_before_io() {
    ...
    let diagnostics = not_started(
        ...
        source_engine::test_support::PhasePreStartFailure::RequestMismatch,
    );
...
fn posting_occurrence(
    ...
    let (reference, identity) =
        source_engine::test_support::validate_posting_reference("fixture", url, None).unwrap();
    PostingOccurrence {
        identity,
        reference,
        provider_values: source_engine::test_support::ProviderValues {
