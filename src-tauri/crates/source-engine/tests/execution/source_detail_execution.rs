...
async fn source_mismatch_precedes_reuse_routing_and_io() {
    ...
    let execution = SourceBehaviorDetailExecution::new(&fetcher, browser_free_acquisition());
...
async fn budget_exhaustion_stays_distinct_and_releases_no_fields_or_dispositions() {
    ...
    let (mut expected_report, expected_diagnostics) = match execute_detail(
        &source_engine::test_support::test_execution_plan(&compiled),
        &Default::default(),
        &occurrence,
...
fn compiled_source(strategies: Vec<Value>) -> source_engine::test_support::CompiledSource {
...
fn compiled_source_with_policy(
    ...
    policy: Value,
) -> source_engine::test_support::CompiledSource {
    ...
    match crate::support::compile_test_source(&source, Some(profile)) {
        CompileSourceOutcome::Compiled {
            ...
            diagnostic.severity != source_engine::test_support::DiagnosticSeverity::Error
        }) =>
        {
            compiled
        }
...
) -> source_engine::test_support::PostingOccurrence {
    let (reference, identity) = source_engine::test_support::validate_posting_reference(
        source_key,
