...
use source_profile_dsl::test_support::{
    execute_detail, BrowserInteractionInstruction, BrowserWaitInstruction, DetailField, Diagnostic,
    DiagnosticCategory, DiagnosticSeverity, HttpMethod, PhaseBrowser, PhaseCompletion,
    PostingOccurrence, RequestedDetailFields, RuntimeCancellation, RuntimeExecutionContext,
    ScriptedHttpBodyEvent, ScriptedHttpEvent, ScriptedProfileHttpClient, SourceBehavior,
...
fn detail_acceptance_rejects_unrequested_fields_before_io() {
    ...
    let diagnostics = not_started(
        ...
        source_profile_dsl::test_support::PhasePreStartFailure::RequestMismatch,
    );
...
fn posting_occurrence(
    ...
    let (reference, identity) =
        source_profile_dsl::test_support::validate_posting_reference("fixture", url, None).unwrap();
    PostingOccurrence {
        identity,
        reference,
        provider_values: source_profile_dsl::test_support::ProviderValues {
