use crate::support::{
    ...
    RegistrySourceProfile, SourceProfileRegistrySnapshot,
};
...
use source_profile_dsl::test_support::{
    ...
    DetailBrowserAdapter, DiscoveryStep, ExecutionPlanFetch, PhaseBrowser, PhaseCompletion,
...
fn final_compiler_preserves_policy_for_inherited_specialized_added_and_source_owned_sets() {
    ...
    assert!(matches!(
        source_profile_dsl::test_support::test_access(&source),
        source_profile_dsl::test_support::CompiledSourceAccess::SourceOwned { .. }
    ));
    assert_plan_policies(
...
fn collect_all_decides_dissatisfaction_only_after_every_strategy_for_both_phases() {
    ...
    assert_eq!(
        terminal.category,
        source_profile_dsl::test_support::DiagnosticCategory::Runtime
    );
    ...
    assert_eq!(
        terminal.severity,
        source_profile_dsl::test_support::DiagnosticSeverity::Error
    );
...
fn all_required_fails_fast_without_reducing_or_exposing_an_accepted_prefix() {
    ...
    let serialized = serde_json::to_string(&PolicyOutcome::<
        source_profile_dsl::test_support::DiscoveryPhasePayload,
    >::PolicyUnsatisfied {
        cause,
    })
    .unwrap();
...
fn first_accepted_exhaustion_adds_one_terminal_after_attempt_diagnostics() {
    ...
    assert_eq!(complete_budget_report.usage.strategy_attempts, 3);
    assert!(!serde_json::to_string(&PolicyOutcome::<
        source_profile_dsl::test_support::DiscoveryPhasePayload,
    >::PolicyUnsatisfied {
        cause: cause.clone()
    })
...
fn detail_browser_1999_ms_compiled_and_caller_limits_are_rejected_without_panic() {
    ...
    detail_plan.strategies[0].fetch = ExecutionPlanFetch::Browser {
        url: source_profile_dsl::test_support::compile_template(
            "https://example.test/detail/browser",
            &source_profile_dsl::test_support::TemplateDescriptor::new(),
        )
        .unwrap(),
        timeout_ms: 1_000,
        waits: Vec::new(),
    ...
    let invalid_plan_diagnostics = not_started(
        ...
        source_profile_dsl::test_support::PhasePreStartFailure::PlanMismatch,
    );
    ...
    let caller_result = execution_failed(
        ...
        source_profile_dsl::test_support::PhaseExecutionFailure::InvalidCallerLimits,
    );
...
fn posting() -> PostingOccurrence {
    let (reference, identity) = source_profile_dsl::test_support::validate_posting_reference(
        "example",
