use crate::support::{RegistrySourceProfile, SourceProfileRegistrySnapshot};
use std::sync::atomic::{AtomicBool, Ordering};

use serde_json::json;
use source_profile_dsl::test_support::{
    compile_source, execute_discovery, AllowanceDimension, AllowanceExhaustion,
    AllowanceLimitSource, BrowserAcquisitionFailure, BrowserAcquisitionFailureKind,
    BrowserAcquisitionRequestSnapshot, BrowserInteractionInstruction, BrowserWaitInstruction,
    CompileSourceOutcome, DiscoveryBrowserAdapter, PhaseBrowser, PhaseCompletion,
...
fn compiled_plan(browser: bool) -> SourceExecutionPlan {
    ...
    match compile_source(&source, &registry) {
        CompileSourceOutcome::Compiled { source, .. } => {
...
fn snapshot(remaining: u64) -> BrowserAcquisitionRequestSnapshot {
    BrowserAcquisitionRequestSnapshot {
        ...
        timeout_ms: 30000,
        waits: vec![BrowserWaitInstruction::Selector {
            selector: "main".into(),
            timeout_ms: 1000,
        }],
        interactions: vec![BrowserInteractionInstruction::ClickIfVisible {
            selector: ".more".into(),
            max_count: 1,
            wait_after_ms: None,
        }],
