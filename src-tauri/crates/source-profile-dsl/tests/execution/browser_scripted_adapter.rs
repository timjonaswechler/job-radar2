use source_profile_dsl::test_support::{
    ...
    BrowserAcquisitionRequestSnapshot, BrowserAcquisitionTerminal, BrowserInteractionInstruction,
...
async fn ordered_script_asserts_the_complete_phase_neutral_request_and_real_effect_debits() {
    let waits = vec![BrowserWaitInstruction::Selector {
        selector: "main".to_string(),
        timeout_ms: 500,
    }];
    let interactions = vec![BrowserInteractionInstruction::ClickIfVisible {
...
async fn scripted_interaction_one_over_is_rejected_before_any_click_debit() {
    let interactions = vec![BrowserInteractionInstruction::ClickUntilGone {
        selector: ".more".to_string(),
        max_count: 2,
        wait_after_ms: None,
    }];
...
async fn mismatched_unexpected_and_missing_calls_are_detected_deterministically() {
    ...
    let missing = ScriptedBrowserAcquisition::new([ScriptedBrowserAcquisitionExpectation {
        ...
        events: vec![ScriptedBrowserAcquisitionEvent::Failure(
            source_profile_dsl::test_support::BrowserAcquisitionFailure::new(
                source_profile_dsl::test_support::BrowserAcquisitionFailureKind::Navigation,
                "navigation failed",
            ),
        )],
        finalization: ScriptedBrowserFinalization::default(),
