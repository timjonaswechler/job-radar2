use crate::compiler_support::{RegistrySourceProfile, SourceProfileRegistrySnapshot};
...
fn base_and_direct_terminals_have_exact_origins_without_metadata() {
    ...
    let source_profile_dsl::test_support::Fetch::Http { method, body, .. } =
        &mut profile.access_paths[0].discovery.strategies[0].fetch
    else {
        panic!("fixture uses HTTP fetch")
    };
    *method = Some(source_profile_dsl::test_support::HttpMethod::Post);
    *body = Some(
...
fn locator_only_fragment_is_a_noop_and_scalar_and_empty_object_are_terminals() {
    let mut profile: SourceProfileDocument = read_fixture("valid/simple-source-profile.json");
    let source_profile_dsl::test_support::Fetch::Http { headers, .. } =
        &mut profile.access_paths[0].discovery.strategies[0].fetch
    else {
        panic!("fixture uses HTTP fetch")
    };
...
fn source_owned_provenance_is_distinct_all_owned_and_minimized() {
    ...
    let CompiledSourceProvenance::SourceOwned { entries } =
        source_profile_dsl::test_support::test_provenance(&compiled)
    else {
...
fn provenance_serialization_is_typed_stable_and_rejects_unknown_shape() {
    let fixture_path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/fixtures/source-behavior/valid/compiled-source-provenance.json");
...
fn compiled_provenance(
    ...
    source_profile_dsl::test_support::test_provenance(&source).clone()
}
