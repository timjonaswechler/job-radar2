...
use source_profile_dsl::{
    execute_detail, CandidateDetailFailure, CompileSourceOutcome, DetailField, Diagnostic,
    DiagnosticCategory, DiagnosticSeverity, PhaseBrowser, PhaseCompletion, PhaseExecutionFailure,
    PhaseExecutionReport, PhaseLimits, PhaseOutcome, PhasePreStartFailure, PhaseRunError,
    PhaseUsage, PolicyOutcome, PolicyUnsatisfiedCause, ProfileDslSourceDetailExecution,
    ProfileHttpFailureKind, ProviderValues, RequestedDetailFields, RequestedFieldDisposition,
...
fn compiler_rejects_empty_detail_capability_shape() {
    ...
    let outcome = crate::support::compile_test_source(&fixture_source(), Some(profile));
...
async fn source_mismatch_precedes_reuse_routing_and_io() {
    ...
    let execution = ProfileDslSourceDetailExecution::new(&fetcher, browser_free_acquisition());
...
async fn budget_exhaustion_stays_distinct_and_releases_no_fields_or_dispositions() {
    ...
    let (mut expected_report, expected_diagnostics) = match execute_detail(
...
async fn strategy_execution_failure_is_candidate_scoped_and_releases_no_fields() {
    ...
    let (expected_report, expected_diagnostics) = match execute_detail(
        &compiled.execution_plan,
        &Default::default(),
        &occurrence,
        RequestedDetailFields::description_text(),
...
async fn cancellation_is_external_and_preserves_started_phase_evidence_without_result() {
    ...
    let raw_cancelled = match execute_detail(
        &compiled.execution_plan,
        &Default::default(),
        &occurrence,
        RequestedDetailFields::description_text(),
...
fn compiled_source(strategies: Vec<Value>) -> source_profile_dsl::CompiledSource {
...
) -> source_profile_dsl::PostingOccurrence {
    let (reference, identity) = source_profile_dsl::validate_posting_reference(
        source_key,
