...
use source_profile_dsl::{
    ...
    ExecutionPlanBrowserInteraction, ExecutionPlanBrowserWait, HttpMethod, PhaseBrowser,
    PhaseCompletion, PostingOccurrence, RequestedDetailFields, RuntimeCancellation,
...
fn detail_acceptance_rejects_unrequested_fields_before_io() {
    ...
    let diagnostics = not_started(
        ...
        source_profile_dsl::PhasePreStartFailure::RequestMismatch,
    );
...
fn posting_occurrence(
    ...
    let (reference, identity) =
        source_profile_dsl::validate_posting_reference("fixture", url, None).unwrap();
    PostingOccurrence {
        identity,
        reference,
        provider_values: source_profile_dsl::ProviderValues {
