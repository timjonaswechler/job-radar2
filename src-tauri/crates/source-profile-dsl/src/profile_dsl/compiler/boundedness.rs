...
fn validate_phase_limits(
    ...
    if has_browser
        && limits.max_duration_ms
            < crate::profile_dsl::documents::limits::BROWSER_TEARDOWN_RESERVE_MS
    {
        diagnostics.push(compiler_error(
            "browser_phase_duration_below_teardown_reserve",
            format!("maxDurationMs must be at least {} when a Strategy uses Browser acquisition", crate::profile_dsl::documents::limits::BROWSER_TEARDOWN_RESERVE_MS),
            format!("{path}/maxDurationMs"),
            serde_json::json!({ "value": limits.max_duration_ms, "minimum": crate::profile_dsl::documents::limits::BROWSER_TEARDOWN_RESERVE_MS }),
        ));
    }
}
