...
use crate::definition::execution_plan::{
...
fn resolve_access_path<'a>(
    ...
    path_key: &str,
    diagnostics: &mut Diagnostics,
) -> Option<&'a crate::definition::documents::ReusableAccessPathDocument> {
...
fn push_compiled_plan_invariant(error: ExecutionPlanBuildError, diagnostics: &mut Diagnostics) {
    diagnostics.push(compiler_error(
        error.code,
        format!(
            "Validated Source Behavior Language could not be converted into a strict Execution Plan: {}",
            error.message
        ),
        error.path,
        error.details,
