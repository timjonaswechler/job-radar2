//! Compiler boundedness checks for declarative Source Behavior Language plans.
//!
...
use crate::definition::documents::fetch::{
...
use crate::definition::documents::{DetailStep, DiscoveryStep, Fetch, PhaseLimits};
use crate::definition::policy::StrategyPolicy;

use super::compiler_error;

pub const MAX_FALLBACK_STRATEGIES: usize = 50;
...
fn validate_phase_limits(
    ...
    if has_browser
        && limits.max_duration_ms
            < crate::definition::documents::limits::BROWSER_TEARDOWN_RESERVE_MS
    {
        diagnostics.push(compiler_error(
            "browser_phase_duration_below_teardown_reserve",
            format!("maxDurationMs must be at least {} when a Strategy uses Browser acquisition", crate::definition::documents::limits::BROWSER_TEARDOWN_RESERVE_MS),
            format!("{path}/maxDurationMs"),
            serde_json::json!({ "value": limits.max_duration_ms, "minimum": crate::definition::documents::limits::BROWSER_TEARDOWN_RESERVE_MS }),
        ));
    }
}
