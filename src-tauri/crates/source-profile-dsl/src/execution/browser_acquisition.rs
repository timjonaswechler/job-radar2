...
use crate::definition::documents::PhaseLimits;
...
/// Execution-owned instruction for one bounded wait in the Browser port.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum BrowserWaitInstruction {
    Selector { selector: String, timeout_ms: u64 },
    NetworkIdle { timeout_ms: u64 },
}

/// Execution-owned instruction for one bounded interaction in the Browser port.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum BrowserInteractionInstruction {
    ClickIfVisible {
        selector: String,
        max_count: u64,
...
impl BrowserWaitInstruction {
    pub(crate) fn from_plan(
        value: &crate::definition::execution_plan::capabilities::ExecutionPlanBrowserWait,
    ) -> Self {
        use crate::definition::execution_plan::capabilities::ExecutionPlanBrowserWait;
        match value {
            ...
            ExecutionPlanBrowserWait::NetworkIdle { timeout_ms } => Self::NetworkIdle {
                timeout_ms: *timeout_ms,
            },
        }
    }
...
impl BrowserInteractionInstruction {
    pub(crate) fn from_plan(
        value: &crate::definition::execution_plan::capabilities::ExecutionPlanBrowserInteraction,
    ) -> Self {
        use crate::definition::execution_plan::capabilities::ExecutionPlanBrowserInteraction;
        match value {
...
pub struct BrowserAcquisitionRequest<'a> {
    pub target: String,
    pub timeout_ms: u64,
    pub waits: Vec<BrowserWaitInstruction>,
    pub interactions: Vec<BrowserInteractionInstruction>,
...
impl<'a> BrowserAcquisitionRequest<'a> {
    pub(crate) fn new(
        target: String,
        timeout_ms: u64,
        waits: Vec<BrowserWaitInstruction>,
        interactions: Vec<BrowserInteractionInstruction>,
        control: RuntimeExecutionContext<'a>,
    ...
    pub fn browser_work_deadline(&self) -> Instant {
        self.work_deadline
    }

    pub fn browser_graceful_deadline(&self) -> Instant {
        self.control
            .browser_graceful_deadline()
            .expect("Browser acquisition always has caller-owned control")
    }

    pub fn browser_force_deadline(&self) -> Instant {
        self.control
            .browser_force_deadline()
            .expect("Browser acquisition always has caller-owned control")
    }

    pub fn browser_handler_deadline(&self) -> Instant {
        self.control
    ...
    #[doc(hidden)]
    pub fn deadline_terminal(&self) -> BrowserAcquisitionTerminal {
    ...
    pub fn admit_navigation(&self) -> Result<(), BrowserAcquisitionTerminal> {
        self.control
    ...
    pub fn admit_wait(&self) -> Result<(), BrowserAcquisitionTerminal> {
        self.control
            .debit(AllowanceCharge {
                logical_waits: 1,
                ..AllowanceCharge::default()
    ...
    pub fn admit_interaction(&self) -> Result<(), BrowserAcquisitionTerminal> {
        self.control
            .debit(AllowanceCharge {
                browser_actions: 1,
                ..AllowanceCharge::default()
...
fn validate_request_bounds(
    timeout_ms: u64,
    waits: &[BrowserWaitInstruction],
    interactions: &[BrowserInteractionInstruction],
    limits: PhaseLimits,
    ...
    for (wait_index, wait) in waits.iter().enumerate() {
        let wait_timeout_ms = match wait {
            BrowserWaitInstruction::Selector { timeout_ms, .. }
            | BrowserWaitInstruction::NetworkIdle { timeout_ms } => *timeout_ms,
        };
...
#[doc(hidden)]
pub async fn probe_browser_acquisition(
...
fn interaction_max_count(interaction: &BrowserInteractionInstruction) -> u64 {
    match interaction {
        BrowserInteractionInstruction::ClickIfVisible { max_count, .. }
        | BrowserInteractionInstruction::ClickUntilGone { max_count, .. } => *max_count,
    }
}

fn interaction_wait_after(interaction: &BrowserInteractionInstruction) -> Option<u64> {
    match interaction {
        BrowserInteractionInstruction::ClickIfVisible { wait_after_ms, .. }
        | BrowserInteractionInstruction::ClickUntilGone { wait_after_ms, .. } => {
