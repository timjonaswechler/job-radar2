...
use crate::definition::template::json_pointer_segment;
...
pub(super) fn extract_candidate(
    ...
    let (reference, identity) = match crate::execution::occurrence::validate_posting_reference(
        source_key,
        &provider_url,
        provider_posting_id,
    ) {
        ...
        Err(error) => {
            let (code, path, reason) = match error {
                crate::execution::occurrence::OccurrenceReferenceError::InvalidUrl =>
                    ("occurrence_reference_invalid", url_path, "invalid_absolute_http_url"),
                crate::execution::occurrence::OccurrenceReferenceError::UserInfo =>
                    ("occurrence_reference_invalid", url_path, "userinfo_forbidden"),
                crate::execution::occurrence::OccurrenceReferenceError::EmptyProviderPostingId =>
                    ("occurrence_provider_id_empty", format!("{base_path}/extract/reference/providerPostingId"), "empty_provider_posting_id"),
                crate::execution::occurrence::OccurrenceReferenceError::FragmentWithoutProviderPostingId =>
    ...
    let locations = provider_values
        ...
        .unwrap_or_default();
    let provider_values = crate::execution::occurrence::ProviderValues {
        title: scalar(
    ...
    for (key, hint) in output.hints.iter().flatten() {
        if let Some(value) = optional_scalar(
            ...
            hints.insert(
                key.clone(),
                crate::execution::occurrence::DiscoveryHint {
                    value,
