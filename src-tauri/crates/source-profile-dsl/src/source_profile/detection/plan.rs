...
use crate::profile_dsl::primitives::capture::{compile_named_pattern, CompiledNamedPattern};
use crate::profile_dsl::primitives::fetch::http::{compile_http_fetch, CompiledHttpFetch};
use crate::profile_dsl::primitives::predicate::{compile_regex, CompiledRegex};
    ...
    macro_rules! row {
        ($key:literal,$owner:expr,$shape:expr,$identity:literal,$witness:expr) => {
            CompiledRegistration {
                ...
                owner: $owner,
                canonical_file:
                    "src-tauri/crates/source-profile-dsl/src/source_profile/detection/plan.rs",
                shape: $shape,
