...
27. Every public Discovery or Detail invocation owns one private cumulative allowance root shared by all Strategies and nested work. The immutable backend ceilings are 50 Strategy attempts, 1,000 logical requests, 100,000 produced items, 120,000 milliseconds, 1,000 pages, 50 Browser actions, and 100,000 fan-out admissions. An optional seven-field `limits` object on a Discovery or Detail Strategy Set may tighten all seven values; Direct Source Specialization and optional caller limits may tighten inherited values further, but zero, `null`, omission/removal of inherited fields, unknown fields, and any raise are rejected rather than clamped. Successful compilation stores mandatory resolved limits in the immutable Execution Plan. Profile DSL Retry is not a phase allowance dimension and remains unsupported.
28. One checked atomic debit evaluates affected dimensions in attempts, requests, produced items, duration, pages, Browser actions, and fan-out order before the effect. Equality succeeds; denied work is neither charged nor started; accepted, rejected, failed, or cancelled work already performed is never refunded. Produced-item and fan-out prefixes are charged deterministically. Cancellation is checked before debit/effect and wins simultaneous deadline readiness. Source Live Check tightens the cumulative Discovery request allowance to one request for its whole invocation, rather than resetting capacity per Strategy. Global security and runtime ceilings cannot be raised or disabled by a Source specialization.
28a. Every Discovery or Detail terminal after work starts carries exactly one `PhaseExecutionReport` containing the seven exact usage counters and one typed completion: accepted, policy-unsatisfied, budget-exhausted, execution-failed, or cancelled. Budget exhaustion records the denied dimension, requested and remaining amounts, and tied backend/compiled/caller limit sources in that order. A pre-start Source/plan mismatch has no report. A caller-limit mismatch returns an `execution-failed` report with all seven usage counters at zero. Budget, internal failure, and Cancellation expose no phase payload, and accounting is never reconstructed from Diagnostics. One validated Detail patch counts as one produced item regardless of field count. This R02 report does not define the later O03 common payload/outcome/commit envelope.
