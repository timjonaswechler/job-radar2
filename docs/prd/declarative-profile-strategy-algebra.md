...
26. Recovered failed attempts remain observable but do not make a later accepted `first_accepted` result fail solely because fallback was exercised. Discovery and Detail execute mandatory compiled `first_accepted` through one crate-private typed ordered-attempt kernel. The kernel owns attempt identity/history and accepted, cancelled, or exhausted terminal selection; phase adapters retain typed inputs and outputs, acceptance, failure classification, reducers, validation, and public Diagnostic projection. HTTP, Browser, and pagination cancellation propagates as typed control state and is translated to a Structured Diagnostic only by the matching phase adapter; Diagnostic codes and text never drive runtime control flow.
