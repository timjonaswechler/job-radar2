...
- Every complete Discovery or Detail Strategy Set requires one closed typed Policy: `{ "type": "first_accepted" }` for ordered fallback or `{ "type": "all_required" }` for strict sequential universal acceptance. Existing-set direct fragments may omit Policy only to inherit the complete Base terminal.
