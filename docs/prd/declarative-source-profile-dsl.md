...
- Every complete Discovery or Detail Strategy Set requires one closed typed Policy: `{ "type": "first_accepted" }` for ordered fallback, `{ "type": "all_required" }` for strict sequential universal acceptance, or `{ "type": "at_least", "count": N }` for threshold acceptance. `N` must be positive and no greater than the final merged Strategy cardinality. Existing-set direct fragments may omit Policy only to inherit the complete Base terminal.
