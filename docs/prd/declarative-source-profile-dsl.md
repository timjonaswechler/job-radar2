...
- **Capture** extracts named values from text using regex named groups. For Discovery and Detail, each outer Capture key selects the regex group with the same name and becomes that output variable; unnamed, `value`, and arbitrary named-group fallback are not part of the DSL contract. Captures may be produced during detection or per selected posting item.
