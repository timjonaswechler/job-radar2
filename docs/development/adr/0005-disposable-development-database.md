...
When a squashed development schema removes a column or lifecycle value, developers use the explicit `just db-reset-after-squash` path. This includes the Posting Occurrence identity hard cut from URL approximation to canonical Source-local provider-ID/normalized-URL identity and the occurrence-owned `is_primary` invariant that replaced `job_postings.primary_source_id`; existing rows are reset rather than assigned invented identities or compatibility pointers. `just db-preserve-after-squash` is only for unchanged schemas and refuses incompatible data; neither path permits a release build to delete a user's database.
