...
| Provider-neutral callers and deterministic tests share one seam | Crate-owned `src-tauri/crates/agent/src/contract_tests/conversation.rs` tests exercise `Conversation` with the regularly compiled `ScriptedProvider`; production Codex tests drive the same conversation interface |
