...
This document catalogs the maintained credential-safety invariant and its executable evidence. Verification must inspect only value-free status and filesystem metadata; it must not inspect or retain credential/configuration contents, environment credentials, account data, prompts, or provider responses.
...
| Provider-neutral callers and deterministic tests share one seam | Crate-owned `src-tauri/crates/agent/src/contract_tests/conversation.rs` tests exercise `AgentConversation` with the regularly compiled `ScriptedProvider`; production Codex tests drive the same conversation interface |
...
| Legacy migration is no-loss, private, and conflict-safe | `src-tauri/crates/agent/tests/contracts/authentication_storage.rs` covers migration, verified `0700`/`0600` modes, redacted conflict handling, canonical-root validation, and symlink rejection |
| Direct keys and exact environment references remain value-free at registry boundaries | `availability_resolves_direct_and_environment_references_without_exposing_values` in `src-tauri/crates/agent/tests/contracts/model_registry.rs`; auth resolution is covered by `api_keys_resolve_direct_values_and_exact_environment_references_deterministically` |
...
| Configuration and Tauri-facing results are redacted | `src-tauri/tests/desktop/agent/configuration_api.rs` serializes status, mutations, invalid-file diagnostics, and login progress and proves synthetic secret/account values are absent; `SecretApiKeyInput` is deserialize-only and non-debuggable |
...
cargo test --manifest-path src-tauri/crates/agent/Cargo.toml openai_codex --no-fail-fast
cargo test --manifest-path src-tauri/crates/agent/Cargo.toml --lib contract_tests::conversation::
cargo test --manifest-path src-tauri/Cargo.toml --test desktop agent::configuration_api::
cargo test --manifest-path src-tauri/crates/agent/Cargo.toml --test contracts authentication_storage::
cargo test --manifest-path src-tauri/crates/agent/Cargo.toml --test contracts model_registry::
