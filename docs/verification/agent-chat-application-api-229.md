...
| Unavailable recorded models remain readable without silent fallback; explicit model and Reasoning Level changes use the latest registry snapshot and return effective state | `identified_resume_preserves_unavailable_model_without_fallback`; `model_selection_uses_the_latest_provider_registry_snapshot` | Resume/settings | Pass |
